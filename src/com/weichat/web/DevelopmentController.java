...
public class DevelopmentController {
	...
	public String developmentMobileShow(HttpServletRequest request, ModelMap modelMap) {
		Double id = Double.parseDouble(request.getParameter("id"));
		Productpeoper pp=developmentService.findProductpeoperById(id);
		Zhengwuqingkuang zw=developmentService.findZhengwuqingkuangById(id);
		Guotu gt=developmentService.findGuotuById(id);
	...
	public void updateproductpeoper(HttpServletResponse response,HttpServletRequest request,@ModelAttribute Productpeoper productpeoper)throws IOException{
		StringBuffer sbResult = new StringBuffer();
		//判断企业安全是否存在
		if(developmentService.checkProductpeoperById(productpeoper.getInfomation().getId())){//存在
			...
			//调用新增方法
			if (developmentService.addNewProductPeoperFromDevelopmentService(productpeoper,productpeoper.getInfomation().getId())){
	...
	public void updateZhengwuqingkuang(HttpServletResponse response,HttpServletRequest request,@ModelAttribute Zhengwuqingkuang zhengwuqingkuang)throws IOException{
		StringBuffer sbResult = new StringBuffer();
		//判断政务办理情况是否存在
		if(developmentService.checkZhengwuqingkuang(zhengwuqingkuang.getInfomation().getId())){//存在
			...
			//调用新增方法
			if (developmentService.addNewGovernmentSituationFromDevelopmentService(zhengwuqingkuang,zhengwuqingkuang.getInfomation().getId())){
				sbResult.append("<script>alert('恭喜！数据已成功录入。'); parent.location.href='../company/companylist.jhtml';</script>");
			} else {
				sbResult.append("<script>alert('非常抱歉，录入数据失败！请重试您的操作。'); parent.location.href='../company/companylist.jhtml'</script>");
			}
	...
	public void updateGuoto(HttpServletResponse response,HttpServletRequest request,@ModelAttribute Guotu guotu)throws IOException{
		StringBuffer sbResult = new StringBuffer();
		//判断政务办理情况是否存在
		if(developmentService.checkGuotu(guotu.getInfomation().getId())){//存在
			...
			//调用新增方法
			if (developmentService.addNewLandSituationFromDevelopmentService(guotu,guotu.getInfomation().getId())){
				sbResult.append("<script>alert('恭喜！数据已成功录入。'); parent.location.href='../company/companylist.jhtml';</script>");
			} else {
				sbResult.append("<script>alert('非常抱歉，录入数据失败！请重试您的操作。'); parent.location.href='../company/companylist.jhtml'</script>");
			}
	...
	public void updateYaosu(HttpServletResponse response,HttpServletRequest request,@ModelAttribute Yaosu yaosu)throws IOException{
		StringBuffer sbResult = new StringBuffer();
		//判断政务办理情况是否存在
		if(developmentService.checkYaosu(yaosu.getInfomation().getId())){//存在
			...
			//调用新增方法
			if (developmentService.addNewElementsOfSecurityFromDevelopmentService(yaosu, yaosu.getInfomation().getId())){
				sbResult.append("<script>alert('恭喜！数据已成功录入。'); parent.location.href='../company/companylist.jhtml';</script>");
			} else {
				sbResult.append("<script>alert('非常抱歉，录入数据失败！请重试您的操作。'); parent.location.href='../company/companylist.jhtml'</script>");
			}
	...
	public void updateJiansejindu(HttpServletResponse response,HttpServletRequest request,@ModelAttribute Jiansejindu jiansejindu)throws IOException{
		StringBuffer sbResult = new StringBuffer();
		//判断政务办理情况是否存在
		if(developmentService.checkJiansejindu(jiansejindu.getInfomation().getId())){//存在
			...
			//调用新增方法
			if (developmentService.addNewProjectBuildToProgressFromDevelopmentService(jiansejindu,jiansejindu.getInfomation().getId())){
				sbResult.append("<script>alert('恭喜！数据已成功录入。'); parent.location.href='../company/companylist.jhtml';</script>");
			} else {
				sbResult.append("<script>alert('非常抱歉，录入数据失败！请重试您的操作。'); parent.location.href='../company/companylist.jhtml'</script>");
			}
