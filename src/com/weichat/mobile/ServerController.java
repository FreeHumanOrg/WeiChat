 ...
 * @version 1.0 Beta
 */
@Controller("serverControllerMobile")
@RequestMapping(value = "/servermobile")
public class ServerController {
	...
	public String serverShow(HttpServletRequest request, ModelMap modelMap) {
		...
		return "/mobile/update/enterprise_update_situation/server";
	}
	...
	public void updateQiyefuwu(HttpServletResponse response,HttpServletRequest request,
		...
		if (serverService.checkQiyefuwu(qiyefuwu.getInfomation().getId())) {// 存在
			...
			if (serverService.addServerByEnterpriseService(qiyefuwu,qiyefuwu.getInfomation().getId())){
				...
				sbResult.append("<script>alert('非常抱歉，录入数据失败！请重试您的操作。'); parent.location.href='../companymobile/mobilelist.jhtml'</script>");
			}
	...
	public void addServer(HttpServletRequest request,
		...
		if (serverService.addServerByEnterpriseService(
			...
			sbResult.append("<script>alert('恭喜！数据已成功录入。'); parent.location.href='../companymobile/companymobile.jhtml';</script>");
		} else {
