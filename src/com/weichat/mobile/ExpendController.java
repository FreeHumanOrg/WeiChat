 ...
 * @version 1.0 Beta
 */
@Controller("expendControllerMobile")
@RequestMapping(value = "/expendmobile")
public class ExpendController {
	...
	public String policyShow(HttpServletRequest request, ModelMap modelMap) {
		...
		return "/mobile/update/enterprise_update_situation/expand";
	}
	...
	public void updateExpend(HttpServletResponse response,HttpServletRequest request,
		...
		LOGGER.info("企业发展修改!"
				+ DateTimeUtils.getNowDateOfStringFormatUsingDateTimeTemplateOne());
	...
	public void addExpend(HttpServletRequest request,
		...
		if (expendService.addNewExpendService(qiyefazhan,Double.valueOf(request.getSession().getAttribute("enterpriseId").toString()))) {
			sbResult.append("<script>alert('恭喜！数据已成功录入。'); parent.location.href='../company/mobilelist.jhtml';</script>");
		} else {
